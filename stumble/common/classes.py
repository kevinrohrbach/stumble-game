"""Define classes."""


# Room Classes ------------------------------------------------ #
class Room():
    """This class represents rooms in the game."""

    def __init__(self, data, room):
        """Hold all the fields for a room."""
        self.name = data['rooms'][room]['name']
        self.descr_light = data['rooms'][room]['descr_light']
        self.descr_dark = data['rooms'][room]['descr_dark']
        self.exits = data['rooms'][room]['exits']
        self.character = None
        self.content = data['rooms'][room]['content']
        self.light = None
        self.visits = False

    def get_name(self):
        """Get current room name."""
        print(self.name)

    def get_details(self):
        """Get full details of room."""
        print(self.name)
        print("----------------")
        print(self.descr_light)


# Character classes ------------------------------------------- #
class Character():
    """This class represents the characters in the game."""

    def __init__(self, state, name, room):
        """Hold all the character attributes."""
        self.name = None  # character name
        self.description = None
        self.room = Room(state, room)
        self.conversation = None
        self.madness = 0  # charter madness (maybe use 1-10)
        self.inventory = []
        # TODO: Add more character traits


class Player(Character):
    """This class represents the player in the game."""

    def __init__(self, state, name, room):
        """Hold extra player attributes."""
        Character.__init__(self, state, name, room)

    def move(self, direction):
        """Define player's movement."""
        if self.room.exits[direction] is None:
            print("Cannot move in that direction!")
            return
        return self.room.exits[direction]


# Item classes --------------------------------------------- #
class Item():
    """This class represents items in the game."""

    def __init__(self):
        """Hold all fields for Item."""
        self.name = ""  # name of the object
        self.description = ""  # description of item to be printed
